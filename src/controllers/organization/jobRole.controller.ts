...
async function addJobRole(req: Request, res: Response): Promise<void> {
  ...
  const { jobTitle, type, minRequiredYearsOfExp,status,isUrgent,startDate,endDate,jobRoleLanguages, jobRoleSkills, ...rest } = req.body;
...
async function getAllJobRoles  (req: Request, res: Response): Promise<void>{
  ...
  try {
    ...
    console.error(error);
     res.status(HttpStatusCodes.INTERNAL_SERVER_ERROR).json({ error: "Failed to retrieve job roles." });
  }
};
...
async function editJobRole(req: Request, res: Response): Promise<void> {
    ...
    const { jobTitle, type, minRequiredYearsOfExp, jobRoleLanguages, jobRoleSkills, ...rest } = req.body;
    ...
    try {
      ...
      res.status(HttpStatusCodes.OK).json({ message: "Job role updated successfully.", jobRole: updatedJobRole });
    } catch (error) {
      console.error(error);
  ...
  async function closeJobRole(req: Request, res: Response): Promise<void> {
    ...
    try {
      const existingJobRole = await prisma.jobRoles.findUnique({ where: { id: jobRoleId } });
      ...
      if (existingJobRole.status === "closed") {
        res.status(HttpStatusCodes.BAD_REQUEST).json({ error: "Job role is already closed and cannot be deleted." });
        return;
      }
      ...
      res.status(HttpStatusCodes.OK).json({ message: "Job role status updated to closed.", jobRole: closedJobRole });
    } catch (error) {
      console.error(error);
      res.status(HttpStatusCodes.INTERNAL_SERVER_ERROR).json({ error: "Error updating job role status." });
    }
  }
